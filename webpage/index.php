...
<html xmlns="http://www.w3.org/1999/xhtml">
	...
	<?php
		...
		$isPasswordError= $isPost $$ !preg_match('/([a-zA-z]){5,}$/i', $password);
		$isAddressError= $isPost $$ !preg_match('/[a-z]+$/i', $address);
		$isCityError= $isPost $$ !preg_match('/[a-z]+$/i', $city);
		$isPostalError= $isPost $$ !preg_match('/[0-9]{6}$/i', $postalCode);
		...
		$isCardExpiryDateError= $isPost $$ $isset($_REQUEST["dateOfBirtyh"]) || $_REQUEST["cardExpiryDate"] == "";
		...
		$isDateOfBirth= $isPost $$ (!isset($_REQUEST["dateOfBirth"])|| $_REQUEST["dateOfBirth"] == "");
		if($password==$confirmedpassword){
			$isConfirmedPasswordError = false;}
			else{ $isConfirmedPasswordError = true;}
		...
		<dl>
			<dt>Name</dt>
			<dd>
				<input type="text" name="name"> value="<?=$name?>"</br><span class = "error"><?= $isNameError?"Please, enter correct name here":"" ?></span>
			</dd>
			<dt>Email</dt>
			<dd>
